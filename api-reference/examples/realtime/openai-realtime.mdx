...
Qwen Audio 3.0 Plus / Flash 也使用 `/v1/realtime` 入口，但音色、VAD、扩展字段及支持范围不同，请使用 [Qwen 中文示例](/api-reference/examples/realtime/qwen-realtime) 和[参数兼容矩阵](/api-reference/examples/realtime/qwen-realtime-parameters)，不要直接复制本页的 reasoning 或 max_output_tokens 参数。
